export const WBOOKS = "WBooks";
export const INGRESAR = "INGRESAR CON GOOGLE";
export const EMAIL = "EMail";
export const PASSWORD = "Password";
export const MAIL_ERROR = "El formato del email no es válido";
export const FORM_INCOMPLETE = "Todos los campos deben estar completos";
export const PASSWORD_SIZE_ERROR =
  "La contraseña debe tener entre 8 y 52 caracteres";
export const PASSWORD_NUMBER_AND_LETTER_ERROR =
  "La contraseña debe contener un número y una letra";
